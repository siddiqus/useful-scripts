...
function getCoverage() {
  try {
    const result = execSync('yarn test --coverage --maxWorkers=2')
      .toString()
      .split('\n');
...
function getCoverageForDiffFiles(baseRef) {
  ...
  if (!diffListWithoutJestIgnore.length) {
    console.info('ℹ️ No diff found, printing coverage report for all');
    return [coverage.all];
  }
  ...
  diffFilesCoverage = diffFilesCoverage.filter((d) => fs.existsSync(d.name));
